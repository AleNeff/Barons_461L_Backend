...
def create_project(name, description, id, owner):
    ...
    """
    project = Project(name, description, id, owner)
    try:
        Project.PROJECTS_COLLECTION.insert_one(project.__dict__)
        return 0
...
def delete_project_by_name(name):
    ...
    Project.PROJECTS_COLLECTION.delete_one({"project_name": name})

def delete_project_by_id(id):
    """Delete a project by project id (str)"""
    Project.PROJECTS_COLLECTION.delete_one({"project_id": id})
...
def check_out_hwset(project_name, hwset_name, amount_out):
    ...
    project_dict = Project.PROJECTS_COLLECTION.find_one({"project_name": project_name})
    project = Project("","","","")
    ...
    Project.PROJECTS_COLLECTION.find_one_and_update(
